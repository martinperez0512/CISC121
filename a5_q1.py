...
def create_standard_rooms():
    ...
    for i in range(1, 90):  # iterate over the amount of rooms
        stand_room = hotel_classes.StandardRoom(i + 100, True)  # create room object
...
def create_suites():
    ...
    for i in range(1, 20):  # iterate over the amount of rooms
        suite_room = hotel_classes.Suite(i + 200, True)  # create room object
...
def create_taylor_suites():
    ...
    for i in range(1, 14):  # iterate over the amount of rooms
        taylor_suite = hotel_classes.TaylorSuite(i + 300, True)  # create room object
...
def main():
    ...
    while True:
        if guest_choice == 1:
            ...
            guest_1 = hotel_classes.Guest(guest_name, guest_check_out_date)  # create guest object

            if guest_room_type == "standard":  # if user chooses standard room
                standard = hotel_classes.StandardRoom(random.choice(standard_rooms), True)  # pick random room
                ...
                suite = hotel_classes.Suite(random.choice(suites), True)  # pick random room
                ...
                taylor_suite = hotel_classes.TaylorSuite(random.choice(taylor_suites), True)  # pick random room
            ...
            snacks = hotel_classes.Service("Snacks", 10)  # create service object
            taxi = hotel_classes.Service("Taxi", 20)  # create service object
            movie = hotel_classes.Service("Movie", 5)  # create service object
            ...
            if "snacks" in guest_service:  # check if user wants this service
                hotel_classes.Guest.add_service(guest_1, snacks)  # add to guest object
            if "taxi" in guest_service:  # check if user wants this service
                hotel_classes.Guest.add_service(guest_1, taxi)  # add to guest object
            if "movie" in guest_service:  # check if user wants this service
                hotel_classes.Guest.add_service(guest_1, movie)  # add to guest object
            ...
            if guest_total == "yes":  # if user wants to see bill. Room costs are not included (professor said it's OK)
                hotel_classes.Guest.send_bill(guest_1)  # call method with guest input and print bill
